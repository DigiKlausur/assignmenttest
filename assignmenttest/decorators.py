...
def test_asserts(points):
    ...
    def catch_all_asserts(method):
        ...
        updated.body[0].body.insert(0, ast.parse("from assignmenttest import grade_report").body[0])

        updated.body[0].body.append(ast.parse(f"print(str({n_passed}) + ' / ' + str({total}) + ' tests passed' )").body[0])
        updated.body[0].body.append(ast.parse("print()").body[0])

        if transformer.n_asserts > 0:        
            updated.body[0].body.append(ast.parse(f"grade_report({n_passed}/{transformer.n_asserts}, {points})").body[0])
        else:
        ...
        from assignmenttest import grade_report
